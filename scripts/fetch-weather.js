...
function isNetworkError(error) { return NETWORK_CODES.has(errorCode(error)); }
...
function getResponse(url) {
  // No external undici package is required. https.request allows a 60-second socket/connect window in a plain Actions runner.
  return new Promise((resolve, reject) => {
    ...
    request.setTimeout(CONNECT_TIMEOUT_MS, () => {
      const timeoutError = new Error(`Request timed out after ${CONNECT_TIMEOUT_MS}ms`);
...
async function requestWithNetworkRetry(endpoint, beachId, beach, base, serviceKey) {
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try { return await requestOnce(endpoint, beachId, beach, base, serviceKey, attempt); }
      ...
      console.log(`[Weather] attempt=${attempt} network error=${errorCode(error)}; retrying in ${wait / 1000}s`);
      await delay(wait);
    }
  }
