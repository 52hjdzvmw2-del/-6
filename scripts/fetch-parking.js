...
function toLot(item, expectedCode) {
  // The requested code is authoritative: this API can omit parkcd in live rows.
  ...
  const updatedAt = item?.lastupdatetime != null ? String(item.lastupdatetime) : null;
  const valid = typeof name === "string" && name.trim() !== "" && Number.isFinite(total) && total >= 0 && Number.isFinite(available) && available >= 0 && typeof updatedAt === "string" && updatedAt.trim() !== "";
  if (!valid) return null;
  let occupied = null;
  if (item?.curvacnt !== undefined && item?.curvacnt !== null && item?.curvacnt !== "") {
    const rawOccupied = Number(item.curvacnt);
    if (Number.isFinite(rawOccupied)) occupied = rawOccupied;
  }
  const dataQuality = available > total || total <= 0 ? "warning" : "ok";
  if (occupied === null && total > 0 && available >= 0 && available <= total) occupied = total - available;
  return { code, name, total, occupied, available, updatedAt, status: dataQuality };
}
async function fetchCode(code) {
  ...
  console.log(`[Parking] content-type=${response.headers.get("content-type") ?? "missing"}`);
...
async function attemptCode(code) {
  try { return await fetchCode(code); }
    ...
    console.error(`[Parking ERROR] code=${code}`);
    console.error("name:", error?.name ?? "UnknownError");
    console.error("message:", error?.message ?? String(error));
    if (error?.cause) {
      console.error("cause code:", error.cause.code ?? "missing");
      console.error("cause message:", error.cause.message ?? "missing");
    }
    return null;
  }
}
...
async function main() {
  if (!process.env.PARKING_API_KEY) throw new Error("PARKING_API_KEY is missing");
